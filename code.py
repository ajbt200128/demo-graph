...
def route_param(route_param):
    ...
    cleaned = clean_thing(route_param)
    url = "https://%s/path" % cleaned
    requests.get(url)

    # ruleid: tainted-url-host
    url = "http://%r/path" % cleaned
    requests.get(url)
...
def route_param_format(route_param):
    print("blah")
    # ruleid: tainted-url-host
    do_thing("https://%s/path" % route_param)
...
def route_param_percent_format(route_param):
    ...
    # ruleid: tainted-url-host
    do_thing("https://%s/path" % route_param)
    do_thing("https://%s/path" % cleaned)
    return "<a href='https://%s/path'>Click me!</a>" % cleaned
...
def post_param_branch():
    ...
    if True:
        ...
        return "<a href='https://%r/path'>Click me also!</a>" % (param,)

@app.route("/post_param_branch", methods=["POST"])
def post_param_branch():
    param = flask.request.form['param']
    param_cleaned = clean_thing(param)
    if True:
        # ruleid: tainted-url-host
        return "<a href='https://%r/path/%r/'>Click me!</a>" % (param,param_cleaned)
